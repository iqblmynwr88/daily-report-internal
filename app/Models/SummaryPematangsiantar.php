...
class SummaryPematangsiantar extends Model
{
    ...
    {
        return SummaryPematangsiantar::where('merchant._id',$merchant,true)->where('year',$year,true)->orderBy('merchant.name', 'asc')->get();
    }
    ...
    public function ExportData($tahun, $bulan, $perangkat , $wilayah)
    {
        ...
        if ($perangkat === "all") {
            $data_ = SummaryPematangsiantar::where('year',$tahun,true)->where('merchant.status',1,true)->orderBy('merchant.name', 'asc')->get();
            foreach ($data_ as $data) {
            ...
            foreach ($data_ as $data) {
                ...
                $data2 = TransactionPematangsiantar::where('merchant.name',$data['merchant']['name'],true)->where('pmt',$perangkat)->take(1)->get();
                foreach ($data2 as $sub2) {
