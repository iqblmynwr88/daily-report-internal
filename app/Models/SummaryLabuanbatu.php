...
class SummaryLabuanbatu extends Model
{
    ...
    public function DetailWilayah($id, $tanggal_awal, $tanggal_akhir)
    {
        ...
        $tax = 0;
        $lembar = 0;
        $detail = TransactionLabuanbatu::where('merchant._id',$id,true)->take(1)->get();
        foreach ($detail as $trxDet) {
            $trx = DetailTrxLabuhanbatu::where('idMerchant',$trxDet['merchant']['_id'])->whereRaw(['trxDate' => ['$gt' => $tanggal_awal, '$lt' => $tanggal_akhir],],true)->get();
            foreach ($trx as $res) {
                $lembar++;
                $amt += $res['trxAmount'];
                $tax += $res['trxTax'];
    ...
    public function GetDataByPmt($tahun, $bulan, $start, $length, $keyword)
    {
        try {
            if ($keyword === "all") {
                ...
                $awal = TransactionLabuanbatu::where('pmt',$keyword)->orderBy('merchant.name', 'asc')->groupBy('merchant')->skip($start)->take($length)->get();
                foreach ($awal as $dt_awal) {
                    ...
                    foreach ($data_ as $data) {
                        $total = TransactionLabuanbatu::where('pmt',$keyword)->groupBy('merchant')->get();
