...
class SummaryMedan extends Model
{
    ...
    public function GetDataByPmt($tahun, $bulan, $start, $length, $keyword)
    {
        try {
            if ($keyword === "all") {
                ...
                $awal = TransactionMedan::where('pmt',$keyword)->orderBy('merchant.name', 'asc')->groupBy('merchant')->skip($start)->take($length)->get();
                foreach ($awal as $dt_awal) {
                    ...
                    foreach ($data_ as $data) {
                        $total = TransactionMedan::where('pmt',$keyword)->groupBy('merchant')->get();
    ...
    public function ExportData($tahun, $bulan, $perangkat , $wilayah)
    {
        ...
        if ($perangkat === "all") {
            ...
            $data_ = SummaryMedan::where('year',$tahun,true)->orderBy('merchant.name', 'asc',true)->get();
            foreach ($data_ as $data) {
