...
class SummaryMedan extends Model
{
    ...
    {
        $data_ = SummaryMedan::where('year',$tahun,true)->orderBy('merchant.name', 'asc')->get();
    ...
    {   
        ...
        $data_ = SummaryMedan::where('year',$tahun,true)->where('merchant._id',$id,true)->orderBy('merchant.name', 'asc')->get();
        foreach ($data_ as $data) {
            $data2 = TransactionMedan::where('merchant.name',$data['merchant']['name'],true)->take(1)->get();
            foreach ($data2 as $sub2) {
                $isi[] = [
