...
class SummaryKaro extends Model
{
    ...
    public function merge($tahun, $bulan, $start, $length, $keyword)
    {
        try {
            if (empty($keyword) || $keyword === "") {
                $total = SummaryKaro::where('year',$tahun,true)->groupBy('merchant')->get();
                $data_ = SummaryKaro::where('year',$tahun,true)->orderBy('merchant.name', 'asc')->skip($start)->take($length)->get();
            } else {
                $total = SummaryKaro::where('year',$tahun,true)->where('merchant.name','like','%'.$keyword.'%')->groupBy('merchant')->get();
                $data_ = SummaryKaro::where('year',$tahun,true)->where('merchant.name','like','%'.$keyword.'%')->orderBy('merchant.name', 'asc')->skip($start)->take($length)->get();
            }
            foreach ($data_ as $data) {
                $data2 = TransactionKaro::where('merchant.name',$data['merchant']['name'],true)->take(1)->get();
    ...
    {   
        SummaryKaro::where('year',$tahun,true)->where('merchant._id',$id,true)->unset($bulan.'.'.'keterangan');
        SummaryKaro::where('year',$tahun,true)->where('merchant._id',$id,true)->push($bulan.'.'.'keterangan',$keterangan);
        MerchantKaro::where('_id',$id,true)->where('name',$nama,true)->update(['status' => $status]);
        TransactionKaro::where('merchant._id',$id,true)->where('merchant.name',$nama,true)->update(['merchant.status' => $status]);
        TransactionKaro::where('merchant._id',$id,true)->where('merchant.name',$nama,true)->update(['status' => $status]);
        SummaryKaro::where('merchant._id',$id,true)->where('merchant.name',$nama,true)->update(['merchant.status' => $status]);
    ...
    public function GetDataByPmt($tahun, $bulan, $start, $length, $keyword)
    {
        try {
            if ($keyword === "all") {
                ...
                foreach ($data_ as $data) {
                    $total = SummaryKaro::where('year',$tahun,true)->groupBy('merchant')->get();
                    $data2 = TransactionKaro::where('merchant.name',$data['merchant']['name'])->take(1)->get();
                    foreach ($data2 as $sub2) {
                        $start++;
                        $isi[] = [
                            'id' => $data['merchant']['_id'],
                ...
                $awal = TransactionKaro::where('pmt',$keyword)->orderBy('merchant.name', 'asc')->groupBy('merchant')->skip($start)->take($length)->get();
                foreach ($awal as $dt_awal) {
                    $data_ = SummaryKaro::where('year',$tahun)->where('merchant.name',$dt_awal['merchant']['name'])->orderBy('merchant.name', 'asc')->get();
                    foreach ($data_ as $data) {
                        $total = TransactionKaro::where('pmt',$keyword)->groupBy('merchant')->get();
                        $data2 = TransactionKaro::where('merchant.name',$data['merchant']['name'])->where('pmt',$keyword)->take(1)->get();
                        foreach ($data2 as $sub2) {
                            $start++;
                            $isi[] = [
                                'id' => $data['merchant']['_id'],
    ...
    {
        return SummaryKaro::where('merchant._id',$merchant,true)->where('year',$year,true)->orderBy('merchant.name', 'asc')->get();
    }

    public function getPerangkat_()
    {
        return TransactionKaro::groupBy('pmt')->get(['pmt']);
    }
    ...
    public function ExportData($tahun, $bulan, $perangkat , $wilayah)
    {
        ...
        if ($perangkat === "all") {
            $data_ = SummaryKaro::where('year',$tahun,true)->where('merchant.status',1,true)->orderBy('merchant.name', 'asc')->get();
            foreach ($data_ as $data) {
            ...
            foreach ($data_ as $data) {
                ...
                $data2 = TransactionKaro::where('merchant.name',$data['merchant']['name'],true)->where('pmt',$perangkat)->take(1)->get();
                foreach ($data2 as $sub2) {
